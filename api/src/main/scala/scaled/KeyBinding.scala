...
/** Defines a key binding. Trigger sequences are defined thusly: A single key consists of the key
  ...
  *
  * The fn bindings are defined by the mode, by using the [[Fn]] annotation on methods. The name in
  * the keymap corresponds to the de-camel-cased method name (see [[Mode]] docs). When a mode
  ...
  * "scala:goto-term").
  */
case class KeyBinding (trigger :String, fn :String)
